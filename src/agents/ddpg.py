...
import utils
from noise.noises import *
from network.networks_head import Actor
from network.networks_head import Critic
...
class DDPG(object):
    def __init__(self, env_dims, lr_actor, lr_critic, device, noise_type, noise_seed):
        ...
        self.actor = Actor(self.state_dim, self.action_dim, self.max_action, 'linear').to(self.device)
        self.actor_target = Actor(self.state_dim, self.action_dim, self.max_action, 'linear').to(self.device)
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr =  lr_actor)

        self.critic = Critic(self.state_dim, self.action_dim, 'linear').to(self.device)
        self.critic_target = Critic(self.state_dim, self.action_dim, 'linear').to(self.device)
    ...
    def train_step(self, experiences, training_steps, batch_size=100, discount=0.99, tau=0.005):
